...
function wcmp_process_callback()
{
    ...
    if (!in_array($client_ip, $safaricom_ips) && !defined('WCMP_SKIP_IP_CHECK')) {
        mpesa_write_log('WCMP: Invalid IP address: ' . $client_ip);
...
function wcmp_verify_payment()
{
    try {
        ...
        $callback_received = get_post_meta($order_id, '_wcmp_callback_received', true);
        $payment_initiated = get_post_meta($order_id, '_wcmp_payment_initiated', true);
