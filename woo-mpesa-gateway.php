...
add_action('plugins_loaded', function() {
    ...
    mpesa_init_payment_gateway();
});

// Helper function for debug logging
function mpesa_write_log($log) {
    /*$options = get_option('woocommerce_mpesa_settings');
...
function wcmp_enqueue_assets() {
    ...
    if (is_checkout()) {
        ...
        wp_enqueue_style(
            ...
            [],
            filemtime(plugin_dir_path(__FILE__) . 'assets/css/mpesa-checkout.css')
        );

        // Phone number validation
        wp_enqueue_script(
            'wcmp-checkout',
            plugins_url('assets/js/mpesa-checkout.js', __FILE__),
            ['jquery'],
            filemtime(plugin_dir_path(__FILE__) . 'assets/js/mpesa-checkout.js'),
            true
        );
